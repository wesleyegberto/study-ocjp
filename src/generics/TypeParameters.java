 ...
 * Type Erasure
 * 
 * > Genéricos foram introduzidos no Java para permitir uma
 *   programação genérica e para permitir ao compilador checagem de tipos mais
 *   rigorosas;
 * > Para implementar genéricos o compilador Java executa um processo,
 *   chamado Type Erasure, para deixar o código genérico e consistente. Essas
 *   operações são:
 *   - substituir todas as ocorrências do type parameters pelos tipos mais específicos.
 *   Ex.: Quando são lower bounded ou ubounded é substituído por Object
 *     List<? super Number> lst => List<Object> lst;
 *     List<T> lst -> List<Object> lst; 
 *   Quando são upper bounded são substituído pelo  supertipo passado
 *     List<? extends Number> lst -> List<Number> lst;
 *   - insere os casts necessários depois de substituir os type parameters
 *   Ex.: O código  abaixo:
 ...
 * 
 * Varargs
 * > Genéricos com varargs pode causar:
 *  - poluição da memória heap
 *  - lançar ClassCastException
 *> Se não aplicado corretamente por causar problemas.
 *  Ex.: <T> void doStuff(T ... arg) {
 *      Object[] b = arg; // Isso permite adicionar qualquer coisa no array e
 *      // posteriormente quando for ler poderá lançar ClassCastException
